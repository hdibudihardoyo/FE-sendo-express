...
const PAYMENT_STATUS_OPTIONS = ["PENDING", "PAID", "SETTLED", "EXPIRED", "FAILED", "REFUNDED"] as const;
...
function normalizePaymentStatus(status?: string): PaymentStatus | undefined {
    ...
    return (PAYMENT_STATUS_OPTIONS as readonly string[]).includes(upper) ? (upper as PaymentStatus) : undefined;
}

function normalizeRecentShipmentsParams(params: RecentShipmentsParams): RecentShipmentsParams {
    return {
...
function normalizeRevenueDays(days: number): number {
    return Number.isFinite(days) ? clampInt(days, REVENUE_DAYS_MIN, REVENUE_DAYS_MAX) : REVENUE_DAYS_DEFAULT;
}
...
export const superAdminDashboardKeys = {
    ...
    revenue: (days: number) => [...superAdminDashboardKeys.all, "revenue", normalizeRevenueDays(days)] as const,
    volumeByBranch: (limit: number) =>
        [...superAdminDashboardKeys.all, "volume-branch", normalizeBranchLimit(limit)] as const,
    statusDistribution: () => [...superAdminDashboardKeys.all, "status-distribution"] as const,
    branchRanking: () => [...superAdminDashboardKeys.all, "branch-ranking"] as const,
    recentShipments: (params: RecentShipmentsParams) =>
        [...superAdminDashboardKeys.all, "recent-shipments", normalizeRecentShipmentsParams(params)] as const,
    alerts: () => [...superAdminDashboardKeys.all, "alerts"] as const,
};
...
export function useDashboardAlerts() {
    return useQuery<DashboardAlertsResponse, Error>({
        queryKey: superAdminDashboardKeys.alerts(),
        queryFn: () => SuperAdminDashboard.getDashboardAlerts(),
        staleTime: 30 * 1000, 
