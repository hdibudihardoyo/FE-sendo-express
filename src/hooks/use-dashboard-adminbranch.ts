...
function normalizeRecentScanLogsParams(params: RecentScanLogsParams): RecentScanLogsParams {
    return {
        page: params.page !== undefined ? clampInt(params.page, SCAN_PAGE_MIN) : undefined,
        limit: params.limit !== undefined ? clampInt(params.limit, SCAN_LIMIT_MIN, SCAN_LIMIT_MAX) : undefined,
    };
}

function normalizeDays(days: number): number {
    return Number.isFinite(days) ? clampInt(days, DAYS_MIN, DAYS_MAX) : DAYS_DEFAULT;
}

function normalizeHours(hours: number): number {
    return Number.isFinite(hours) ? clampInt(hours, HOURS_MIN, HOURS_MAX) : HOURS_DEFAULT;
}
...
export const adminBranchDashboardKeys = {
    ...
    recentScanLogs: (params: RecentScanLogsParams) =>
        [...adminBranchDashboardKeys.all, "recent-scan-logs", normalizeRecentScanLogsParams(params)] as const,
    activityTrend: (days: number) =>
        [...adminBranchDashboardKeys.all, "activity-trend", normalizeDays(days)] as const,
    stuckPackagesAlert: (hours: number) =>
        [...adminBranchDashboardKeys.all, "stuck-packages-alert", normalizeHours(hours)] as const,
};
...
export function useStuckPackagesAlert(hours: number) {
    const normalizedHours = normalizeHours(hours);

    return useQuery<StuckPackagesAlertResponse, Error>({
        queryKey: adminBranchDashboardKeys.stuckPackagesAlert(normalizedHours),
        queryFn: () => AdminBranchDashboard.getStuckPackagesAlert(normalizedHours),
        staleTime: 30 * 1000,
