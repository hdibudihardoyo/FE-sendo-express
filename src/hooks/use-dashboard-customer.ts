...
import { customerDashboard, type paramsCustomer } from "@/lib/api/services/dashboard-customer"; 
import type {
...
function normalizeShipmentHistoryParams(params: paramsCustomer): paramsCustomer {
    return {
...
export const customerDashboardKeys = {
    ...
    activeShipments: () => [...customerDashboardKeys.all, "active-shipments"] as const,
    shipmentHistory: (params: paramsCustomer) =>
        [...customerDashboardKeys.all, "shipment-history", normalizeShipmentHistoryParams(params)] as const,
};

export function useActiveShipments() {
    return useQuery<ActiveShipmentsResponse, Error>({
        queryKey: customerDashboardKeys.activeShipments(),
        queryFn: () => customerDashboard.getActiveShipments(),
        staleTime: 30 * 1000, 
